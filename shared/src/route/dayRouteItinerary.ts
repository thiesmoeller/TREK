...
import { BUILTIN_ROUTE_MODES } from './route-mode.schema';

export type RouteLegMode = string;
...
export interface DayRouteItinerary {
  /** Place-only runs split at transport breakpoints (for OSRM / plugin routing). */
  runs: DayRouteRun[];
  /** Straight-line polylines including transport endpoint anchors (map preview). */
  straightSegments: [number, number][][];
}

const BUILTIN_ROUTE_MODE_SET = new Set<string>(BUILTIN_ROUTE_MODES);

/** Normalize a trip default; non-empty values pass through, empty falls back to a built-in. */
export function normalizeRouteMode(
  value: unknown,
  fallback: unknown = 'walking',
): string {
  const s = typeof value === 'string' ? value.trim().toLowerCase() : '';
  if (s) return s;
  const fb = typeof fallback === 'string' ? fallback.trim().toLowerCase() : '';
  if (BUILTIN_ROUTE_MODE_SET.has(fb)) return fb;
  return 'walking';
}

/** `inherit` (and empty) → null; any other non-empty string is kept as the override. */
export function parseRouteModeOverride(value: unknown): string | null {

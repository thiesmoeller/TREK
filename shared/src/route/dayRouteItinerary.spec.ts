...
describe('parseRouteModeOverride', () => {
  it('maps inherit and empty values to null', () => {
    expect(parseRouteModeOverride('inherit')).toBeNull();
    expect(parseRouteModeOverride(null)).toBeNull();
    expect(parseRouteModeOverride('')).toBeNull();
    expect(parseRouteModeOverride(undefined)).toBeNull();
  });

  it('accepts any non-empty mode string', () => {
    expect(parseRouteModeOverride('waterway')).toBe('waterway');
    expect(parseRouteModeOverride('  Driving  ')).toBe('driving');
    expect(parseRouteModeOverride('custom-mode')).toBe('custom-mode');
  });
});

describe('normalizeRouteMode', () => {
  it('passes through non-empty values including plugin modes', () => {
    expect(normalizeRouteMode('driving')).toBe('driving');
    expect(normalizeRouteMode('waterway')).toBe('waterway');
  });

  it('falls back to a built-in only when the value is empty', () => {
    expect(normalizeRouteMode(null, 'driving')).toBe('driving');
    expect(normalizeRouteMode('', 'driving')).toBe('driving');
    expect(normalizeRouteMode(null, 'waterway')).toBe('walking');
    expect(normalizeRouteMode(null, null)).toBe('walking');
  });
});
...
describe('effectiveRouteMode', () => {
  it('uses assignment override, then trip default, then walking', () => {
    ...
    expect(effectiveRouteMode('inherit', 'driving')).toBe('driving');

...
export class TripsController {
  ...
  create(@CurrentUser() user: User, @Body() body: Record<string, unknown>, @Req() req: Request) {
    ...
    const drkRaw: string | null | undefined =
      typeof body.default_route_mode === 'string'
        ? body.default_route_mode
        : body.default_route_mode === null
          ? null
          : undefined;
    const waterwaySpeedRaw = body.waterway_speed_kmh;
    const waterwaySpeedKmh = waterwaySpeedRaw === null || waterwaySpeedRaw === undefined
      ? undefined
      : Number(waterwaySpeedRaw);
    const { trip, tripId, reminderDays } = this.trips.create(user.id, {
      ...
      ...(drkRaw !== undefined ? { default_route_mode: drkRaw } : {}),
      ...(waterwaySpeedKmh !== undefined && !Number.isNaN(waterwaySpeedKmh)
        ? { waterway_speed_kmh: waterwaySpeedKmh }
        : {}),
    });
  ...
  async update(@CurrentUser() user: User, @Param('id') id: string, @Body() body: Record<string, unknown>, @Req() req: Request, @Headers('x-socket-id') socketId?: string) {
    ...
    const editFields = [
      'title', 'description', 'start_date', 'end_date', 'currency', 'reminder_days', 'day_count',
      'default_route_mode', 'waterway_speed_kmh',
    ];

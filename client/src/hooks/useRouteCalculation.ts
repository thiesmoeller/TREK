...
import { formatRouteLegPill, formatDistanceM, formatDurationS } from '../utils/formatRouteLeg'
...
function waterwaySpeedKmhFromTrip(trip: { route_mode_options?: Record<string, Record<string, unknown>> | null } | null): number {
  const speed = trip?.route_mode_options?.waterway?.speedKmh
  return typeof speed === 'number' && speed > 0 ? speed : 6
}
...
async function fetchAllDayRoutesParallel(
  ...
  options: {
    ...
    buildRouteInputForDay: (dayId: number) => {
      ...
      straightDisplay: [number, number][][]
      bookends: RouteBookends
    }
    mapRouteLegs: (legs: DayRouteLeg[], polylineOffset?: number) => RouteSegment[]
    useClientBookends: boolean
  },
): Promise<{ allSegments: [number, number][][]; allLegs: RouteSegment[] } | { aborted: true }> {
  const { signal, buildRouteInputForDay, mapRouteLegs, useClientBookends } = options
  const entries = dayIds
    .map(id => ({ id, routeInput: buildRouteInputForDay(id) }))
    .map(e => ({
      id: e.id,
      straightForDay: useClientBookends ? e.routeInput.straightDisplay : e.routeInput.straightSegments,
      fallbackStraight: e.routeInput.straightDisplay,
    }))
  ...
  async function worker() {
    while (cursor < entries.length) {
      ...
      const idx = cursor++
      const { id, straightForDay, fallbackStraight } = entries[idx]
      try {
        ...
        byId.set(id, geo?.segments?.length
          ? { segments: geo.segments, legs: geo.legs || [] }
          : { segments: fallbackStraight, legs: [] })
      } catch (err: unknown) {
        if (err instanceof Error && err.name === 'AbortError') return
        if (signal.aborted) return
        byId.set(id, { segments: fallbackStraight, legs: [] })
      }
    }
  }
 ...
 * When route calculation is on, uses the server mixed-route geometry API; otherwise
 * keeps straight-line segments with optional client-side accommodation bookends.
 */
export function useRouteCalculation(
  ...
  const tripRouteOptionsSig = useTripStore((s) => JSON.stringify((s.trip as { route_mode_options?: unknown } | null)?.route_mode_options ?? {}))
  ...
  const buildRouteInputForDay = useCallback((dayId: number): {
    ...
    return {
      straightSegments,
      straightDisplay: addAccommodationBookendsToStraightSegments(straightSegments, bookends),
      geocodedWaypoints,
      bookends,
    }
  }, [accommodations, optimizeFromAccommodation])

  const buildStraightDisplayForDay = useCallback(
    (dayId: number): [number, number][][] => buildRouteInputForDay(dayId).straightDisplay,
    [buildRouteInputForDay],
  )

  const mapRouteLegs = useCallback((legs: DayRouteLeg[], polylineOffset = 0): RouteSegment[] => {
    const trip = useTripStore.getState().trip as { route_mode_options?: Record<string, Record<string, unknown>> | null } | null
    const waterwaySpeedKmh = waterwaySpeedKmhFromTrip(trip)
    return (legs || []).map(l => {
      ...
      return {
        ...
        durationS: l.durationS,
        isApproximate: l.isApproximate,
        distanceText: formatDistanceM(l.distanceM),
        durationText: formatDurationS(l.durationS),
      }
    })
  }, [t])
  ...
  const updateRouteForDay = useCallback(async (dayId: number | null) => {
    ...
    if (!dayId) {
      ...
      const straightByDay = dayIds.flatMap(id => (
        routeCalcEnabled ? buildRouteInputForDay(id).straightSegments : buildStraightDisplayForDay(id)
      ))
    ...
    const { straightSegments, straightDisplay, geocodedWaypoints } = buildRouteInputForDay(dayId)

    if (straightDisplay.length === 0 && geocodedWaypoints.length < 2) {
      setRoute(null)
      setRouteSegments([])
      return
    }

    const baseline = routeCalcEnabled ? straightSegments : straightDisplay
    setRoute(baseline.length > 0 ? baseline : null)
    ...
    try {
      ...
      } else if (straightDisplay.length > 0) {
        setRoute(straightDisplay)
        setRouteSegments([])
      }
    } catch (err: unknown) {
      if (err instanceof Error && err.name === 'AbortError') return
      if (controller.signal.aborted) return
      if (straightDisplay.length > 0) setRoute(straightDisplay)
      setRouteSegments([])
    }
  }, [buildRouteInputForDay, buildStraightDisplayForDay, mapRouteLegs, routeCalcEnabled, routeShown])
  ...
  }, [selectedDayId, selectedDayAssignments, transportSignature, assignmentRouteSig, tripRouteDefaultSig, tripRouteOptionsSig, tripId, routeShown, accommodations, optimizeFromAccommodation, distanceUnit])

...
import type { Coordinate, OverpassClient, WaterwayLegResult } from './types';

export interface RouteWaterwayLegOptions {
  overpassClient: OverpassClient;
  legKey?: string;
  signal?: AbortSignal;
  snapMaxM?: number;
  bboxPadM?: number;
  cache?: Map<string, { at: number; elements: unknown[] }>;
  cacheTtlMs?: number;
  cacheMax?: number;
}

type OsmNodeEl = { type: string; id: number; lat: number; lon: number };
type OsmWayEl = { type: string; id: number; nodes: number[]; tags?: Record<string, string> };

const defaultCache = new Map<string, { at: number; elements: unknown[] }>();

function trimCache(cache: Map<string, { at: number; elements: unknown[] }>, cacheMax: number): void {
...
export function buildUndirectedWaterwayEdges(elements: unknown[]): {
  pos: Map<string, Coordinate>;
  edges: Map<string, { a: string; b: string; w: number }>;
} {
  const pos = new Map<string, Coordinate>();
  const edges = new Map<string, { a: string; b: string; w: number }>();

  for (const el of elements) {
    const e = el as OsmNodeEl;
...
function buildAdjacency(edges: Map<string, { a: string; b: string; w: number }>): Map<string, Array<{ id: string; w: number }>> {
  const adjacency = new Map<string, Array<{ id: string; w: number }>>();
...
function shortestPath(edges: Map<string, { a: string; b: string; w: number }>, start: string, end: string): string[] | null {
...
function pathToCoords(pos: Map<string, Coordinate>, path: string[]): [number, number][] {
...
function pathDistanceM(edges: Map<string, { a: string; b: string; w: number }>, path: string[]): number {
  let d = 0;
...
export async function routeWaterwayLeg(
  from: Coordinate,
  to: Coordinate,
  options: RouteWaterwayLegOptions,
